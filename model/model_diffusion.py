...
class Model_diffusion_id_v_con_a(torch.nn.Module):
    def __init__(self, args, item_num, text_content=None, pretrained_embs=None, audio_embs=None):
        super(Model_diffusion_id_v_con_a, self).__init__()
        self.args = args
    ...
    def forward(self, sample_items_id, log_mask, local_rank, args):
        ...
        diffu_log_feats=torch.cat([prec_vec_noise.unsqueeze(1), prec_vec_a.unsqueeze(1), times_info_embedding.unsqueeze(1)],dim=1)
    ...
    def predict(self, input_embs, id_embs, lvlm_embs, a_embs, log_mask, args):
        ...
        for i in reversed(range(0, self.timesteps)):
            ...
            diffu_log_feats = torch.cat([prec_vec.unsqueeze(1), prec_vec_a.unsqueeze(1), times_info_embeddings.unsqueeze(1)], dim=1)
...
class Model_diffusion_id_a_con_v(torch.nn.Module):
    def __init__(self, args, item_num, text_content=None, pretrained_embs=None, audio_embs=None):
        super(Model_diffusion_id_a_con_v, self).__init__()
        self.args = args
        self.max_seq_len = args.max_seq_len
        self.w = args.w
        self.item_num = item_num
