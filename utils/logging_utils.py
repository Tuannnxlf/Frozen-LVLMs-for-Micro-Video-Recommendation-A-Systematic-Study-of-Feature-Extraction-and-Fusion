...
def setuplogger(dir_label, log_paras, run_id, rank):
    formatter = logging.Formatter('[%(levelname)s %(asctime)s] %(message)s')
    Log_file = logging.getLogger('Log_file')
    Log_screen = logging.getLogger('Log_screen')

    if rank in [-1, 0]:
        log_path = os.path.join('./logs/logs_' + dir_label)
        if not os.path.exists(log_path):
            os.makedirs(log_path)

        log_file_name = os.path.join(log_path, 'log_' + log_paras + run_id + '.log')
